...
q1_traj = Sinusoidal()
q1_traj.updateTrajectory(currentPosition=0,goalPosition=math.pi/3,currentTime=0,timeDuration=1)
q2_traj = Sinusoidal()
q2_traj.updateTrajectory(currentPosition=0,goalPosition=math.pi/6,currentTime=0,timeDuration=1)
###
...
while(True):
    ...
    if world.getWorldTime() > 1:
      desiredPosition[0] = math.pi/3
      desiredPosition[1] = math.pi/6
      desiredVelocity = np.array([0.0,0.0])
